...
useDerivativePredictor = false;
	...
	float aimKi;
	float aimKd;
	bool  aimNoiseEnabled;
	float aimNoiseAmplitude;
	float aimPredictionWeightX;
...
float aimOutputMax;
	...
	int   shuwuMaxRetry;
	int   shuwuTargetDelayMs;
	float shuwuDirectProb;
	float shuwuOvershootProb;
	float shuwuMicroOvsProb;
	float shuwuDyMinRatio;
	float shuwuDyDefaultRatio;
	float shuwuDyUpperLimit;
...
obs_properties_add_group(props, "aim_controller_group", "aim 控制器配置", OBS_GROUP_NORMAL, aimProps);
		...
		// ========== 书屋控制器配置（AiMod 完全一致：MotionSimulator + P_PID） ==========
	obs_properties_t *shuwuProps = obs_properties_create();
	obs_property_t *swKpXProp = obs_properties_add_float_slider(shuwuProps, "shuwu_kp_x", "X轴-Kp", 0.0, 100.0, 0.5);
	obs_property_set_long_description(swKpXProp, "X比例系数（AiMod KpX=25）");
	obs_property_t *swKiXProp = obs_properties_add_float_slider(shuwuProps, "shuwu_ki_x", "X轴-Ki", 0.0, 5.0, 0.01);
	obs_property_set_long_description(swKiXProp, "X积分系数（AiMod KiX=0）");
	obs_property_t *swKdXProp = obs_properties_add_float_slider(shuwuProps, "shuwu_kd_x", "X轴-Kd", 0.0, 100.0, 0.5);
	obs_property_set_long_description(swKdXProp, "X微分系数（AiMod KdX=KpX）");
	obs_property_t *swPredictXProp = obs_properties_add_float_slider(shuwuProps, "shuwu_predict_x", "X轴-Predict", 0.0, 5.0, 0.1);
	obs_property_set_long_description(swPredictXProp, "X输出速度倍率（AiMod PredictX=2）");
	obs_property_t *swRateXProp = obs_properties_add_float_slider(shuwuProps, "shuwu_rate_x", "X轴-Rate", 0.001, 0.5, 0.005);
	obs_property_set_long_description(swRateXProp, "X kp积分速率（AiMod RateX=0.15）");
	obs_property_t *swKpYProp = obs_properties_add_float_slider(shuwuProps, "shuwu_kp_y", "Y轴-Kp", 0.0, 100.0, 0.5);
	obs_property_set_long_description(swKpYProp, "Y比例系数（AiMod KpY=30）");
	obs_property_t *swKiYProp = obs_properties_add_float_slider(shuwuProps, "shuwu_ki_y", "Y轴-Ki", 0.0, 5.0, 0.01);
	obs_property_set_long_description(swKiYProp, "Y积分系数（AiMod KiY=0）");
	obs_property_t *swKdYProp = obs_properties_add_float_slider(shuwuProps, "shuwu_kd_y", "Y轴-Kd", 0.0, 100.0, 0.5);
	obs_property_set_long_description(swKdYProp, "Y微分系数（AiMod KdY=KpY）");
	obs_property_t *swPredictYProp = obs_properties_add_float_slider(shuwuProps, "shuwu_predict_y", "Y轴-Predict", 0.0, 5.0, 0.1);
	obs_property_set_long_description(swPredictYProp, "Y输出速度倍率（AiMod PredictY=0）");
	obs_property_t *swRateYProp = obs_properties_add_float_slider(shuwuProps, "shuwu_rate_y", "Y轴-Rate", 0.001, 0.5, 0.005);
	obs_property_set_long_description(swRateYProp, "Y kp积分速率（AiMod RateY=0.1）");
	obs_property_t *swKiModeProp = obs_properties_add_int(shuwuProps, "shuwu_ki_mode", "KiMode 积分模式", 0, 2, 1);
	obs_property_set_long_description(swKiModeProp, "0=I累加 1=I项开 其它=关");
	obs_property_t *swKpLimitProp = obs_properties_add_float_slider(shuwuProps, "shuwu_kp_limit", "Kp限幅", 0.0, 10000.0, 100.0);
	obs_property_set_long_description(swKpLimitProp, "P项atan2软限幅，0=关");
	obs_property_t *swKiLimitProp = obs_properties_add_float_slider(shuwuProps, "shuwu_ki_limit", "Ki限幅", 0.0, 10000.0, 100.0);
	obs_property_set_long_description(swKiLimitProp, "I项atan2软限幅，0=关");
	obs_property_t *swKdLimitProp = obs_properties_add_float_slider(shuwuProps, "shuwu_kd_limit", "Kd限幅", 0.0, 10000.0, 100.0);
	obs_property_set_long_description(swKdLimitProp, "D项atan2软限幅，0=关");
	obs_property_t *swLimitProp = obs_properties_add_float_slider(shuwuProps, "shuwu_limit", "总输出限幅", 0.0, 10000.0, 100.0);
	obs_property_set_long_description(swLimitProp, "总输出atan2软限幅，0=关");
	obs_property_t *swKiRateProp = obs_properties_add_float_slider(shuwuProps, "shuwu_ki_rate", "KiRate 卡尔曼Q", 0.001, 0.5, 0.001);
	obs_property_set_long_description(swKiRateProp, "ki积分速率→kf1.Q");
	obs_property_t *swDeadbandProp = obs_properties_add_float_slider(shuwuProps, "shuwu_ki_deadband", "KiDeadband 积分死区", 0.0, 2.0, 0.05);
	obs_property_set_long_description(swDeadbandProp, "I项输出死区");
	// MotionSimulator 拟人仿真参数
	obs_property_t *swRetryProp = obs_properties_add_int(shuwuProps, "shuwu_max_retry", "最大重试次数", 0, 5, 1);
	obs_property_set_long_description(swRetryProp, "过冲重试次数（AiMod=2）");
	obs_property_t *swDelayProp = obs_properties_add_int(shuwuProps, "shuwu_target_delay_ms", "目标延迟ms", 0, 500, 10);
	obs_property_set_long_description(swDelayProp, "目标锁定延迟（AiMod=80）");
	obs_property_t *swDirectProp = obs_properties_add_float_slider(shuwuProps, "shuwu_direct_prob", "直线概率", 0.0, 1.0, 0.01);
	obs_property_set_long_description(swDirectProp, "直接瞄准概率（AiMod=0.85）");
	obs_property_t *swOvsProp = obs_properties_add_float_slider(shuwuProps, "shuwu_overshoot_prob", "过冲概率", 0.0, 1.0, 0.01);
	obs_property_set_long_description(swOvsProp, "过冲瞄准概率（AiMod=0.10）");
	obs_property_t *swMicroProp = obs_properties_add_float_slider(shuwuProps, "shuwu_micro_ovs_prob", "微过冲概率", 0.0, 1.0, 0.01);
	obs_property_set_long_description(swMicroProp, "微过冲概率（AiMod=0.05）");
	obs_property_t *swDyMinProp = obs_properties_add_float_slider(shuwuProps, "shuwu_dy_min_ratio", "垂直偏移最小比例", 0.0, 1.0, 0.01);
	obs_property_set_long_description(swDyMinProp, "瞄点垂直偏移下限（AiMod=0.4）");
	obs_property_t *swDyDefProp = obs_properties_add_float_slider(shuwuProps, "shuwu_dy_default_ratio", "垂直偏移默认比例", 0.0, 1.0, 0.01);
	obs_property_set_long_description(swDyDefProp, "瞄点垂直偏移默认（AiMod=0.65）");
	obs_property_t *swDyUpProp = obs_properties_add_float_slider(shuwuProps, "shuwu_dy_upper_limit", "垂直偏移上限", 0.0, 1.0, 0.01);
	obs_property_set_long_description(swDyUpProp, "瞄点垂直偏移上限（AiMod=0.8）");
...
// DerivativePredictor参数默认值（不重复写 prediction_weight，沿用上面 0.3/0.1）
    ...
    obs_data_set_default_double(settings, "shuwu_kp_x", 25.0);
    obs_data_set_default_double(settings, "shuwu_ki_x", 0.0);
    obs_data_set_default_double(settings, "shuwu_kd_x", 25.0);
    obs_data_set_default_double(settings, "shuwu_predict_x", 2.0);
    obs_data_set_default_double(settings, "shuwu_rate_x", 0.15);
    obs_data_set_default_double(settings, "shuwu_kp_y", 30.0);
    obs_data_set_default_double(settings, "shuwu_ki_y", 0.0);
    obs_data_set_default_double(settings, "shuwu_kd_y", 30.0);
    obs_data_set_default_double(settings, "shuwu_predict_y", 0.0);
    obs_data_set_default_double(settings, "shuwu_rate_y", 0.1);
    ...
    obs_data_set_default_int(settings, "shuwu_max_retry", 2);
    obs_data_set_default_int(settings, "shuwu_target_delay_ms", 80);
    obs_data_set_default_double(settings, "shuwu_direct_prob", 0.85);
    obs_data_set_default_double(settings, "shuwu_overshoot_prob", 0.10);
    obs_data_set_default_double(settings, "shuwu_micro_ovs_prob", 0.05);
    obs_data_set_default_double(settings, "shuwu_dy_min_ratio", 0.4);
    obs_data_set_default_double(settings, "shuwu_dy_default_ratio", 0.65);
    obs_data_set_default_double(settings, "shuwu_dy_upper_limit", 0.8);
...
tf->aimOutputMax = (float)obs_data_get_double(settings, "aim_output_max");
		...
		tf->shuwuKpX = (float)obs_data_get_double(settings, "shuwu_kp_x");
		tf->shuwuKiX = (float)obs_data_get_double(settings, "shuwu_ki_x");
		tf->shuwuKdX = (float)obs_data_get_double(settings, "shuwu_kd_x");
		tf->shuwuPredictX = (float)obs_data_get_double(settings, "shuwu_predict_x");
		tf->shuwuRateX = (float)obs_data_get_double(settings, "shuwu_rate_x");
		tf->shuwuKpY = (float)obs_data_get_double(settings, "shuwu_kp_y");
		tf->shuwuKiY = (float)obs_data_get_double(settings, "shuwu_ki_y");
		tf->shuwuKdY = (float)obs_data_get_double(settings, "shuwu_kd_y");
		tf->shuwuPredictY = (float)obs_data_get_double(settings, "shuwu_predict_y");
		tf->shuwuRateY = (float)obs_data_get_double(settings, "shuwu_rate_y");
		...
		tf->shuwuMaxRetry = (int)obs_data_get_int(settings, "shuwu_max_retry");
		tf->shuwuTargetDelayMs = (int)obs_data_get_int(settings, "shuwu_target_delay_ms");
		tf->shuwuDirectProb = (float)obs_data_get_double(settings, "shuwu_direct_prob");
		tf->shuwuOvershootProb = (float)obs_data_get_double(settings, "shuwu_overshoot_prob");
		tf->shuwuMicroOvsProb = (float)obs_data_get_double(settings, "shuwu_micro_ovs_prob");
		tf->shuwuDyMinRatio = (float)obs_data_get_double(settings, "shuwu_dy_min_ratio");
		tf->shuwuDyDefaultRatio = (float)obs_data_get_double(settings, "shuwu_dy_default_ratio");
		tf->shuwuDyUpperLimit = (float)obs_data_get_double(settings, "shuwu_dy_upper_limit");
...
// === 共享指针：消费推理结果（带 age gate + generation） ===
	...
	auto applyConfigToController = [&tf](int configIndex) {
		...
		mcConfig.shuwuKpX = tf->shuwuKpX;
		mcConfig.shuwuKiX = tf->shuwuKiX;
		mcConfig.shuwuKdX = tf->shuwuKdX;
		mcConfig.shuwuPredictX = tf->shuwuPredictX;
		mcConfig.shuwuRateX = tf->shuwuRateX;
		mcConfig.shuwuKpY = tf->shuwuKpY;
		mcConfig.shuwuKiY = tf->shuwuKiY;
		mcConfig.shuwuKdY = tf->shuwuKdY;
		mcConfig.shuwuPredictY = tf->shuwuPredictY;
		mcConfig.shuwuRateY = tf->shuwuRateY;
		...
		mcConfig.shuwuMaxRetry = tf->shuwuMaxRetry;
		mcConfig.shuwuTargetDelayMs = tf->shuwuTargetDelayMs;
		mcConfig.shuwuDirectProb = tf->shuwuDirectProb;
		mcConfig.shuwuOvershootProb = tf->shuwuOvershootProb;
		mcConfig.shuwuMicroOvsProb = tf->shuwuMicroOvsProb;
		mcConfig.shuwuDyMinRatio = tf->shuwuDyMinRatio;
		mcConfig.shuwuDyDefaultRatio = tf->shuwuDyDefaultRatio;
		mcConfig.shuwuDyUpperLimit = tf->shuwuDyUpperLimit;
		// 贝塞尔曲线移动参数
		...
		// aim 控制器参数同步到控制器配置
		mcConfig.aimKp = tf->aimKp;
		mcConfig.aimKi = tf->aimKi;
		mcConfig.aimKd = tf->aimKd;
		mcConfig.aimNoiseEnabled = tf->aimNoiseEnabled;
		mcConfig.aimNoiseAmplitude = tf->aimNoiseAmplitude;
		mcConfig.aimPredictionWeightX = tf->aimPredictionWeightX;
		mcConfig.aimPredictionWeightY = tf->aimPredictionWeightY;
		mcConfig.aimRampTime = tf->aimRampTime;
		mcConfig.aimInitScale = tf->aimInitScale;
		mcConfig.aimOutputMax = tf->aimOutputMax;
		// 神经网络轨迹生成器参数
