...
项目: OBS Background Removal Plugin

这组模板把 Vibe Coding 的快速表达方式和 SCALE 的工程闭环合在一起：先澄清目标，再编排 skills/MCP/CLI，再产出可验证证据。

> Synced from scale-engine src/prompts/VibeTemplateGallery.ts (v0.50.x).
> Reference: scale-engine/docs/VIBE-TEMPLATES.md
...
```text
请作为 CEO 和产品负责人，主导 OBS Background Removal Plugin 的产品发现工作。
...
### 引导问题
- 用户在 OBS 属性面板中最频繁完成的操作是什么？
- 界面应该更像专业工具面板、简洁参数列表，还是高级配置界面？
- 哪些状态必须完整设计：空态、加载、错误、权限不足、多配置切换？

### 复制使用

```text
请作为 UX Director 和高级视觉设计负责人，主导 OBS Background Removal Plugin 的 UI/UX 方案。

场景：请描述本次要解决的问题、目标用户和期望产出
我当前身份：项目负责人

请按 SCALE 工作流执行：
1. explore：阅读现有产品、OBS 属性面板、组件、品牌和设计系统，识别当前视觉语言。
2. plan：输出 UI-SPEC，包含信息架构、核心用户路径、组件状态、OBS 面板适配规则、可访问性要求。
3. build：只给出可执行设计方案或实现切片，不要写营销式空话。
4. verify：要求截图、OBS 界面检查、控制台/网络检查和适配证据。
...
- 每个关键页面包含状态、布局、交互、OBS 面板适配和验收规则。
- 明确需要哪些截图证据证明体验达标。
```
...
- 哪些契约一旦变更会影响其他模块、OBS API 或模型推理？
- 失败、回滚、兼容、观测和权限边界如何设计？

### 复制使用

```text
请作为 CTO 和首席架构师，主导 OBS Background Removal Plugin 的技术实现架构方案。
...
安全边界：
- 不允许编造调用链、接口或测试结果。
- 不允许绕过 OBS API、模型推理接口约定、日志脱敏、安全校验和权限边界。
...
### 引导问题
- 这项工作需要哪些公司角色：CEO/PM/架构/C++开发/QA/安全/发布/文档？
- 哪些角色必须互审，哪些角色只是按需参与？
...
```text
请作为 COO、Engineering Director 和 Agent Orchestrator，主导 OBS Background Removal Plugin 的 Agentic 公司化协作流程。
...
Agent 编排要求：
- 使用现有 agent presets：product-agent、architect-agent、cpp-agent、test-agent、security-agent、performance-agent、docs-agent、ops-agent、code-review-agent。
...
```text
请作为 Technical Program Manager 和 Agent Runtime Lead，编排 OBS Background Removal Plugin 的多 Agent 治理式交付。
...
- cpp-agent：实现切片（C++ / CMake / ONNX Runtime）。
- test-agent：TDD、回归、边界条件。
...
```text
请作为 Quality Director 和 Red Team Lead，对 OBS Background Removal Plugin 执行互审红队与自我修正闭环。
...
- 使用 gates status、runtime evidence、测试命令、截图证据或发布检查命令。
...
```text
请作为 Autonomous Delivery Lead 和 Cost Controller，推进 OBS Background Removal Plugin 的长任务，但必须严格控制预算和闭环质量。
...
- 用 git diff、测试命令、构建命令和 gates status 形成可审计证据。
...
```text
请作为 Engineering Lead，主导 OBS Background Removal Plugin 的实现切片。
...
- 推荐工具: cmake --build build_x64, ctest --test-dir build_x64, git diff --check
...
```text
请作为 QA Lead 和 Release Manager，主导 OBS Background Removal Plugin 的验收与发版前检查。
